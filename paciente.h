    ...
    for(i=0;i<cont+1;i++){
        if(atendidos[i].hs.hora_ate!=24){
                ...
                if(atendidos[i].hs.min_ing<atendidos[i].hs.min_ate || atendidos[i].hs.min_ing == atendidos[i].hs.min_ate){
                    atendidos[i].esp.min_esp=  atendidos[i].hs.min_ing-atendidos[i].hs.min_ate;
...
void cargaArchivoAten(FILE *f, int cont){
    ...
    for(i=0;i<cont;i++){
        if(atendidos[i].hs.hora_ate!=24){
            ...
            fprintf(f,"D.N.I: %ld\n", atendidos[i].dni);
            fprintf(f,"Edad: %d\n", atendidos[i].edad);
            fprintf(f, "Hora de ingreso: %d:%dhs.", atendidos[i].hs.hora_ing, atendidos[i].hs.min_ing);
            fprintf(f, "Hora de atencion: %d:%dhs.", atendidos[i].hs.hora_ate, atendidos[i].hs.min_ate);
        }
    }
}
...
void cargaArchivoAtenUrg(FILE *fp1, FILE *fp2, FILE *fp3, int cont){
    ...
    for (i=0;i<cont;i++){
        if (atendidos[i].nvUrg==1){
            ...
            fprintf(fp1,"Edad: %d\n", atendidos[i].edad);
            fprintf(fp1, "Hora de ingreso: %d:%dhs.", atendidos[i].hs.hora_ing, atendidos[i].hs.min_ing);
            fprintf(fp1, "Hora de atencion: %d:%dhs.", atendidos[i].hs.hora_ate, atendidos[i].hs.min_ate);
        }
    }
    for (i=0;i<cont;i++){
